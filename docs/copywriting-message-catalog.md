...
- 默认显示：
  ...
  - `哎呀呀好痒呀好痒呀！`
  - `喵喵喵꜀(^. .^꜀  )꜆੭`
- 当前触发条件/时间：左键按下后未拖动并松开；如果正在睡觉则按下时优先触发唤醒。按住拖动不会触发摸头动作或气泡。 同一 key 下多条文案会随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。
- 动作：`clicked`
- 修改后文案：
- 修改后触发条件/时间：
...
- 默认显示：
  ...
  - `(*^ω^*)开心`
  - `cchh，嘟嘟哒哒⌯ᵔᗜᵔ⌯`
- 当前触发条件/时间：右键点“开心一下”；随机待机也可能触发开心动作并随机显示其中一条气泡。 同一 key 下多条文案会随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。
- 动作：`happy`
- 修改后文案：
- 修改后触发条件/时间：
...
- 默认显示：
  ...
  - `呆呆最最最喜欢麻麻啦˶>ᗜ<˶`
  - `真的不和呆呆玩一下嘛ₒ⦁⩊⦁ₒ`
- 当前触发条件/时间：右键点“卖萌一下”；随机待机可能播放卖萌动作，但只有主动菜单会显示这组气泡。 同一 key 下多条文案会随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。
- 动作：`cute`
- 修改后文案：
- 修改后触发条件/时间：
...
- 默认显示：
  - `麻麻，看这里呀。`
  - `你好呀，我是呆呆~`
- 当前触发条件/时间：右键点“打个招呼”，或双击呆呆。 同一 key 下多条文案会随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。
- 动作：`wave`
- 修改后文案：
- 修改后触发条件/时间：
...
- 默认显示：
  - `ᶻz ₍^_ ̫ _^₎`
- 当前触发条件/时间：右键点“睡一会儿”。 同一 key 下多条文案会随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。
- 动作：`sleep_in -> sleep`
- 修改后文案：
- 修改后触发条件/时间：
...
- 默认显示：
  - `呆呆醒啦՞･∞･՞`
- 当前触发条件/时间：呆呆处于睡眠动作时左键点击。 同一 key 下多条文案会随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。
- 动作：`wake`
- 修改后文案：
- 修改后触发条件/时间：
...
- 默认显示：
  - `天才在左。`
- 当前触发条件/时间：右键点“向左走两步”。 同一 key 下多条文案会随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。
- 动作：`walk_left`
- 修改后文案：
- 修改后触发条件/时间：
...
- 默认显示：
  - `疯子在右。`
- 当前触发条件/时间：右键点“向右走两步”。 同一 key 下多条文案会随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。
- 动作：`walk`
- 修改后文案：
- 修改后触发条件/时间：
...
- 当前触发条件/时间：第一次运行时，启动约 1.2 秒后显示；正常只显示一次；气泡最长显示 10 秒，招手动作结束时同步消失。
- 动作：`wave`
- 修改后文案：
- 修改后触发条件/时间：
...
特殊日子优先于全部普通语料。非低打扰状态下，程序会把当前时间段语料与`miss_you`、`busy_support`、`comfort`、`encouragement` 通用语料合并，再从冷却结束的候选中随机选择一条；气泡最长显示 3 秒，动作先结束时同步消失。低打扰状态不播放自动陪伴语料。`cooldown_hours` 表示同一条语料再次出现前至少等待多少小时。
