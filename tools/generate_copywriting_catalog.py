...
def write_copywriting_catalog() -> None:
    ...
    for index, key in enumerate(TEXT, 1):
        ...
        line(
            ...
            f"气泡最长显示 {SHORT_BUBBLE_HIDE_MS // 1000} 秒，动作先结束时同步消失。"
        )
        line(f"- 动作：`{action}`")
        line("- 修改后文案：")
        line("- 修改后触发条件/时间：")
    ...
    state_items = [
        (
            ...
            f"第一次运行时，启动约 1.2 秒后显示；正常只显示一次；"
            f"气泡最长显示 {FIRST_LAUNCH_HIDE_MS // 1000} 秒，招手动作结束时同步消失。",
            "wave",
        ),
        (
            "low_distraction_on",
    ...
    line(
        ...
        f"再从冷却结束的候选中随机选择一条；气泡最长显示 {COMPANION_MESSAGE_HIDE_MS // 1000} 秒，"
        "动作先结束时同步消失。"
